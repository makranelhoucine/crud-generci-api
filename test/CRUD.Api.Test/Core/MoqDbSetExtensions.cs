namespace CRUD.Api.Test.Core
{
    ...
    internal static class MoqDbSetExtensions
    {
            ...
            where TEntity : class
        {
            ...
            mock.Setup(m => m.FindAsync(It.IsAny<CancellationToken>(), It.IsAny<object[]>()))
                .Returns<CancellationToken, object[]>((tocken, objs) => Task.Run(() => find(objs), tocken));

            mock.Setup(m => m.Create()).Returns(() => Activator.CreateInstance<TEntity>());

            mock.Setup(m => m.Remove(It.IsAny<TEntity>())).Callback<TEntity>(
                entity =>
                    {
                        data.Remove(entity);
            ...
            mock.Setup(m => m.RemoveRange(It.IsAny<IEnumerable<TEntity>>())).Callback<IEnumerable<TEntity>>(
                entities =>
                    {
            ...
            mock.Setup(m => m.Add(It.IsAny<TEntity>())).Callback<TEntity>(
                entity =>
                    {
                        data.Add(entity);
                        mock.SetupData(data, find);
            ...
            mock.Setup(m => m.Attach(It.IsAny<TEntity>())).Callback<TEntity>(
                entity =>
                    {
                        data.Add(entity);
                        mock.SetupData(data, find);
            ...
            mock.Setup(m => m.AddRange(It.IsAny<IEnumerable<TEntity>>())).Callback<IEnumerable<TEntity>>(
                entities =>
                    {
                        foreach (var entity in entities)
                        {
