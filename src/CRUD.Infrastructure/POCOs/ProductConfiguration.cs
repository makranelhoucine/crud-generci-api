...
namespace CRUD.Infrastructure.POCOs
{
    ...
    public partial class ProductConfiguration : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<Product>
    {
        ...
        public ProductConfiguration(string schema)
        {
            ...
            Property(x => x.Id).HasColumnName(@"ProductID").HasColumnType("int").IsRequired().HasDatabaseGeneratedOption(System.ComponentModel.DataAnnotations.Schema.DatabaseGeneratedOption.Identity);
            ...
            HasOptional(a => a.ProductModel).WithMany(b => b.Products).HasForeignKey(c => c.ProductModelId).WillCascadeOnDelete(false);
            HasOptional(a => a.ProductSubcategory).WithMany(b => b.Products).HasForeignKey(c => c.ProductSubcategoryId).WillCascadeOnDelete(false);
            HasOptional(a => a.UnitMeasure_SizeUnitMeasureCode).WithMany(b => b.Products_SizeUnitMeasureCode).HasForeignKey(c => c.SizeUnitMeasureCode).WillCascadeOnDelete(false);
            HasOptional(a => a.UnitMeasure_WeightUnitMeasureCode).WithMany(b => b.Products_WeightUnitMeasureCode).HasForeignKey(c => c.WeightUnitMeasureCode).WillCascadeOnDelete(false);
            InitializePartial();
        }
