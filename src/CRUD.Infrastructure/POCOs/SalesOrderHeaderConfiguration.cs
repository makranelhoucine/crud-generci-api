...
namespace CRUD.Infrastructure.POCOs
{
    ...
    public partial class SalesOrderHeaderConfiguration : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<SalesOrderHeader>
    {
        ...
        public SalesOrderHeaderConfiguration(string schema)
        {
            ...
            HasOptional(a => a.CreditCard).WithMany(b => b.SalesOrderHeaders).HasForeignKey(c => c.CreditCardId).WillCascadeOnDelete(false);
            HasOptional(a => a.CurrencyRate).WithMany(b => b.SalesOrderHeaders).HasForeignKey(c => c.CurrencyRateId).WillCascadeOnDelete(false);
            HasOptional(a => a.SalesPerson).WithMany(b => b.SalesOrderHeaders).HasForeignKey(c => c.SalesPersonId).WillCascadeOnDelete(false);
            HasOptional(a => a.SalesTerritory).WithMany(b => b.SalesOrderHeaders).HasForeignKey(c => c.TerritoryId).WillCascadeOnDelete(false);
            HasRequired(a => a.BillToAddress).WithMany(b => b.SalesOrderHeaders_BillToAddressId).HasForeignKey(c => c.BillToAddressId).WillCascadeOnDelete(false);
            HasRequired(a => a.Customer).WithMany(b => b.SalesOrderHeaders).HasForeignKey(c => c.CustomerId).WillCascadeOnDelete(false);
            HasRequired(a => a.ShipMethod).WithMany(b => b.SalesOrderHeaders).HasForeignKey(c => c.ShipMethodId).WillCascadeOnDelete(false);
            HasRequired(a => a.ShipToAddress).WithMany(b => b.SalesOrderHeaders_ShipToAddressId).HasForeignKey(c => c.ShipToAddressId).WillCascadeOnDelete(false);
            InitializePartial();
        }
